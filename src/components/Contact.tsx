...
export default function Contact() {
  const [activeTab, setActiveTab] = useState<ProgramId>("personal");
      ...
      <div className="relative z-10 max-w-[1280px] mx-auto px-6">
        <div className="max-w-[650px] mx-auto text-center mb-12 reveal">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-navy/5 rounded-full mb-5">
            ...
            <span className="text-[11px] font-black text-navy uppercase tracking-[0.15em]">
              Inquiry Form
            </span>
          </div>
          <h2 className="text-[clamp(32px,4.5vw,54px)] text-navy leading-[1.05] mb-4">
            Start your student&apos;s{" "}
            <span className="text-coral italic">journey here.</span>
          </h2>
          <p className="text-[17px] text-ink-soft leading-relaxed">
            Select a program below to see available sessions and pricing
            details. We usually respond to all inquiries within one school day.
          </p>
        </div>
          ...
          <div className="inline-flex p-1.5 bg-navy/5 backdrop-blur-sm rounded-[24px] border border-navy/5 shadow-inner">
            {Object.values(PROGRAMS).map((p) => (
              <button
                ...
                className={`relative px-6 py-3 rounded-[20px] text-[14px] font-bold transition-all duration-300 flex items-center gap-2 ${
                  activeTab === p.id
                    ? "bg-white text-navy shadow-sm scale-105"
                    : "text-navy/50 hover:text-navy/80 hover:bg-white/50"
                } ${p.featured ? "bg-green/5 ring-1 ring-green/10" : ""}`}
              >
                {p.featured && (
                  <span className="absolute -top-2 -right-2 px-1.5 py-0.5 bg-coral text-white text-[9px] font-black uppercase tracking-tighter rounded-md shadow-sm animate-bounce">
                    Popular
                  </span>
                )}
                <div
                  className={`w-1.5 h-1.5 rounded-full ${activeTab === p.id ? `bg-${p.accent}` : "bg-transparent"}`}
                />
                {p.label}
              </button>
            ))}
        ...
        <div className="grid lg:grid-cols-[1fr_400px] gap-8 reveal items-stretch">
          <div className="bg-white rounded-[32px] border border-line shadow-xl shadow-navy/5 overflow-hidden flex flex-col min-h-full transition-all duration-500">
            <div
              className={`h-1.5 w-full bg-${activeConfig.accent} transition-colors duration-500`}
            />
            ...
            <div className="p-8 lg:p-10 flex-1 flex flex-col relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col h-full">
                <div className="flex items-center justify-between mb-8 lg:mb-12">
                  <div>
                    <h3 className="text-[28px] lg:text-[34px] font-display font-bold text-navy leading-tight">
                      Program Inquiry ✉️
                    </h3>
                  ...
                  <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-green/10 rounded-full">
                    <span className="w-2 h-2 bg-green rounded-full animate-pulse" />
                    <span className="text-[11px] font-black text-green uppercase tracking-widest">
                      Fast Reply
                    </span>
                  </div>
                </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="relative rounded-[32px] overflow-hidden bg-linear-to-br from-[#1E114D] to-[#3B1977] px-8 py-10 shadow-lg shadow-navy/10 group">
              ...
              <div className="relative z-10">
                <div className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-white/10 backdrop-blur-sm rounded-full border border-white/10 mb-6">
                  ...
                  <span className="text-[#f2a93b] text-[11px] font-black uppercase tracking-widest">
                    Summer 2026
                  </span>
                </div>
                ...
                <p className="text-white/70 text-[14px] leading-relaxed mb-8 pr-4">
                  Every seat is limited. Fill out the form and we&apos;ll
            ...
            <div className="bg-white rounded-lg border border-line shadow-sm overflow-hidden">
              <div className="px-5 pt-5 pb-3 border-b border-line">
                <p className="text-[10px] font-black text-ink-mute uppercase tracking-[0.15em]">
                  Our team
                </p>
              </div>
              <div className="divide-y divide-line">
                {(
                  ...
                  <div
                    key={i}
                    className="px-5 py-4 flex items-center gap-4 hover:bg-background/60 transition-colors duration-150"
                  >
                    {/* Avatar */}
                    <div className="relative w-10 h-10 rounded-full shrink-0 overflow-hidden border border-line shadow-sm">
                      {c.image ? (
                        <Image
                          src={c.image}
                          alt={c.name}
                        ...
                        <div
                          className="w-full h-full grid place-items-center text-white text-[11px] font-black"
                          style={{
                            background: `linear-gradient(135deg, ${c.accent}cc, ${c.accent})`,
                          }}
                        >
                          {c.initials}
                        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-display font-bold text-[14px] text-navy leading-tight">
                          {c.name}
                        </span>
                        <span
